...
fn sensor_thread_main(txes: Vec<Sender<usize>>, tids: Vec<i32>)
{
    ...
    loop {
        ...
        urb.send(MsgStruct {
            ...
            exectime: Duration::from_millis(200).as_nanos() as u64,
        }).unwrap();

        txes[0].send(400).unwrap();
        ...
        urb.send(MsgStruct {
            tid: tids[1],
            relative_deadline: Duration::from_millis(200).as_nanos() as u64,
            exectime: Duration::from_millis(50).as_nanos() as u64,
        }).unwrap();

        txes[1].send(50).unwrap();
        ...
        urb.send(MsgStruct {
            tid: tids[2],
            relative_deadline: Duration::from_millis(500).as_nanos() as u64,
            exectime: Duration::from_millis(200).as_nanos() as u64,
        }).unwrap();

        txes[2].send(200).unwrap();
    }
}
